<template>
  <OverlayDialog :show="show" :close="close" title="Obtain Tokens!">
    <div v-if="tokenSymbol === 'TEER' || tokenSymbol === 'DOT'" class="mt-2">
    ...
    <div v-else class="mt-2">
      <p class="text-sm text-gray-400">
        You can bridge {{ tokenSymbol }} from Ethereum using
        <a href="https://app.snowbridge.network">Snowbridge</a>
      </p>
      <p class="text-sm text-gray-400">
        bridge {{ tokenSymbol }} to this address:
      </p>
      <div class="flex flex-col mt-5">
        <div class="relative flex items-center rounded-lg">
          <input
