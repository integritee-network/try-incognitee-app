  ...
  <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-gray-800 rounded-lg p-6 max-w-md w-full border border-gray-700">
      ...
      <div class="flex justify-between items-center mb-4">
        ...
        <button @click="$emit('close')" class="text-gray-400 hover:text-white p-1 rounded-full hover:bg-gray-700">
      ...
      <!-- Wallet Connection -->
      <div class="mb-4">
        <div v-if="!accountStore.account" class="flex flex-col items-center p-3 border border-gray-700 rounded-lg">
          <p class="text-sm text-gray-400 mb-2">Connect your wallet to purchase credits</p>
          <button @click="connectWallet" class="btn btn-outline">
            <font-awesome-icon :icon="['fas', 'wallet']" class="mr-2" /> Connect Wallet
          </button>
        </div>
        <div v-else class="flex items-center">
          <div>
            <div class="text-sm font-medium">Wallet Connected</div>
            <div class="text-xs text-gray-400" id="modal-wallet-address">{{ accountStore.getAddress }}</div>
          </div>
        </div>
      </div>
      ...
      <div v-if="loading" class="text-center py-4">
        <font-awesome-icon :icon="['fas', 'spinner']" spin class="mr-2" /> Loading packages...
      </div>
      <div v-else class="space-y-3">
        <div 
          ...
          :class="selectedPackage?.id === pkg.id ? 'border-incognitee-green bg-gray-700' : 'border-gray-700 hover:border-gray-500'"
          @click="selectPackage(pkg)"
        >
          <div class="flex justify-between items-center">
            <h3 class="font-medium text-lg">{{ pkg.name }}</h3>
            <span v-if="pkg.bestValue" class="bg-incognitee-green text-white text-xs px-2 py-1 rounded">Best Value</span>
          </div>
      ...
      >
        ...
        <span v-else-if="checkoutLoading">
          <font-awesome-icon :icon="['fas', 'spinner']" spin class="mr-2" /> Processing...
        </span>
        <span v-else>
          Purchase Credits
        </span>
...
import { createCheckoutSession, fetchCreditPackages, formatPrice, type CreditPackage } from '~/utils/stripe';
...
async function connectWallet() {
  try {
    ...
    } else if (typeof accountStore.setAccount === 'function' && !accountStore.account) {
...
async function checkout() {
  ...
  try {
    ...
    localStorage.setItem('incognitee_credit_amount', selectedPackage.value.amount.toString());
    
    // Store wallet address and original URL in localStorage for persistence across redirects
    if (accountStore.account) {
      console.log('Saving wallet address to localStorage before redirect:', accountStore.getAddress);
      localStorage.setItem('incognitee_wallet_address', accountStore.getAddress);
    }
    ...
    const checkoutUrl = await createCheckoutSession(selectedPackage.value.priceId);
    ...
    if (checkoutUrl) {
      ...
      alert('Stripe checkout has opened in a new tab. Please complete your payment there.');
    }
  } catch (error) {
