<template>
 <div class="px-5 py-5 sm:py-5 lg:px-5 lg:py-5">
    ...
    <p class="my-4 mb-10 text-sm text-gray-400">
      Welcome to the FAQ section for Incognitee, your blockchain-based solution for private transfers and messages on the Polkadot network. Below, you'll find answers to the most common questions about our product, its features, and how it can benefit you.
    </p>
    <div v-for="(section, index) in faqSections" :key="index" class="mt-8">
  <h3 class="flex text-lg font-bold tracking-tight text-green-400 sm:text-lg mb-5">
    {{ section.title }}
  </h3>
  ...
  <dl class="space-y-6 divide-y divide-white/10 px-4 py-2">
    <Disclosure as="div" v-for="faq in section.questions" :key="faq.question" v-slot="{ open }">
      <dt>
        <DisclosureButton class="flex w-full justify-between text-left text-white my-5">
...
<div class="flex mt-5 text-xl font-bold tracking-tight text-white sm:text-xl">
  Need More Help?
    ...
    <p class="my-4 mb-10 text-sm text-gray-400">
      If you have any additional questions or need support, please contact us at <a 
      href="mailto:support@incognitee.io"
      class="text-incognitee-green hover:underline"
    >support@integritee.network</a> or via the full documentation at<a 
      ...
      faqSections: [
        {
          ...
          questions: [
            ...
            {
              question: "Why did you choose Polkadot as the underlying blockchain?",
              answer:
        ...
        },
        {
          ...
          questions: [
            {
              question: "How does Incognitee guarantee the security of transactions?",
              answer:
