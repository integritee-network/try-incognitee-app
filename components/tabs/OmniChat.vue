<template>
  <div class="flex flex-1 overflow-hidden">
    ...
    <aside
      class="w-[260px] bg-gray-800 flex flex-col shadow-lg rounded-md h-[calc(100vh-6rem)]"
    >
      <div class="border-b border-base-300 p-4">
        <button
          class="btn btn_gradient inline-flex w-full justify-center rounded-md text-sm font-semibold text-white shadow-sm h-8 min-h-0 py-0"
      ...
      <div class="overflow-y-auto flex-1 p-4 space-y-4 sidebar-scroll">
        ...
        >
          <div v-if="props.groupedChats[section].length > 0">
            ...
            <ul class="space-y-1 px-0.5">
              ...
              >
                <div
                  class="flex items-center w-full rounded-md border border-gray-800 bg-gray-800 py-1.5 px-3 text-gray-400 hover:border-incognitee-green hover:bg-gray-700 focus-within:border-incognitee-green cursor-pointer my-1 text-[14px]"
                  :class="[
                    props.selectedChatId === chat.id
                      ? 'ring-1 border border-blue-500'
                      : '',
        ...
        <template
          v-for="(monthGroup, month) in props.groupedChats.byMonth"
          :key="month"
        >
          <div class="space-y-2">
            <div class="text-xs font-medium text-base-content/70 px-2">
              {{ month }}
            </div>
            <div
              v-for="chat in monthGroup"
              :key="chat.id"
              @click="emit('selectChat', chat.id)"
            ...
            >
              <div class="flex justify-between items-start gap-2">
                ...
                <div
                  class="opacity-0 group-hover:opacity-100 flex items-center space-x-1 shrink-0"
                >
                  <button
                    @click.stop="emit('shareChat', chat.id)"
                  ...
                  <button
                    @click.stop="emit('archiveChat', chat.id)"
    ...
    <!-- Main Content Area -->
    <main
      class="flex-1 h-[calc(100vh-6rem)] flex flex-col overflow-hidden pr-0 pb-0"
    >
      <!-- Chat Messages -->
      <div
        ref="chatContainer"
        class="flex-1 overflow-y-auto p-4 pr-0 space-y-4 chat-container"
      >
        <div
        ...
        >
          ...
          >
            <div
              class="chat-bubble text-white break-words"
              :class="{
      ...
      <div class="p-4 pb-0 pr-0 mt-auto">
        <div
          class="w-[100%] max-w-2xl p-2 bg-transparent rounded-2xl flex flex-col border border-gray-800 shadow-sm"
        >
          <div class="space-y-0">
            <textarea
              v-model="userInput"
            ...
            <div class="flex justify-between items-center m-0">
              ...
              <div class="form-control min-w-[100px]">
                <select
                  v-model="props.selectedProvider"
                  @change="
                    $emit('update:selectedProvider', $event.target.value)
                  "
