...
const selectedProvider = useLocalStorage<Provider>('omnichat-provider', 'openai')
...
const groupedChats = computed(() => {
  ...
  return {
    today: nonArchivedChats.filter(chat => 
      new Date(chat.lastInteraction).getTime() >= today.getTime()
    ),
    ...
    byMonth: nonArchivedChats
      .filter(chat => new Date(chat.lastInteraction).getTime() < thirtyDaysAgo.getTime())
      .reduce((acc, chat) => {
        const date = new Date(chat.lastInteraction)
...
onMounted(() => {
  const existingChats = chatHistory.value.filter(chat => chat.title.startsWith('Chat '))
