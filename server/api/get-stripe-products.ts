...
export default defineEventHandler(async () => {
  try {
    ...
    const creditPackages = prices.data.map(price => {
      ...
      const amount = product.metadata.credits ? parseInt(product.metadata.credits) : 100;
      const bonusAmount = product.metadata.bonus_credits ? parseInt(product.metadata.bonus_credits) : undefined;
    ...
    return createError({
      statusCode: 500,
      message: error instanceof Error ? error.message : 'Failed to fetch Stripe products'
    });
  }
});
