    ...
    if (session.payment_status === 'paid') {
        ...
        if (lineItem.price && lineItem.price.product && typeof lineItem.price.product !== 'string') {
          ...
          if ('deleted' in product && product.deleted) {
            ...
            if (validProduct.metadata && validProduct.metadata.credit_amount) {
              console.log('Found credit_amount in metadata:', validProduct.metadata.credit_amount);
            ...
            else if (validProduct.name && validProduct.name.match(/\d+\s*credits/i)) {
            ...
            else if (validProduct.description && validProduct.description.match(/\d+\s*credits/i)) {
      ...
      try {
        ...
        if (creditAmount > 0) {
          ...
          console.log('Payment verification and credit issuance complete:', result);
          return result;
        } else {
        ...
        return {
          ...
          customerId: session.customer as string,
          paymentIntentId: session.payment_intent as string,
          error: error instanceof Error ? error.message : 'Failed to issue credits'
